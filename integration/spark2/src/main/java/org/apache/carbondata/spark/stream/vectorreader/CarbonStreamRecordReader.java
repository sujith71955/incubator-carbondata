...
package org.apache.carbondata.spark.stream.vectorreader;
...
public class CarbonStreamRecordReader extends RecordReader<Void, Object> {
  ...
  private boolean scanBlockletAndFillVector(BlockletHeader header) throws IOException {
    ...
    if (skipScanData) {
      ...
      vectorProxy= new CarbonVectorProxy(MemoryMode.OFF_HEAP,rowNums,outputSchema);
      vectorProxy.setNumRows(rowNums);
      input.skipBlockletData(true);
      return rowNums > 0;
    }
    input.readBlockletData(header);
    vectorProxy= new CarbonVectorProxy(MemoryMode.OFF_HEAP,input.getRowNums(),outputSchema);
    int rowNum = 0;
    if (null == filter) {
      while (input.hasNext()) {
        readRowFromStream();
  ...
  private void putRowToColumnBatch(int rowId) {
    for (int i = 0; i < projection.length; i++) {
      ...
      vectorProxy.putRowToColumnBatch(vectorProxy.column(i), rowId, value);
